        ...
        ctMethodToPatch.instrument(new ExprEditor() {
            ...
            public void edit(MethodCall m) throws CannotCompileException
            {
                if (m.getMethodName().equals("renderDynamicVariable")) {
                    ...
                    m.replace("$_ = basemod.patches.com.megacrit.cardcrawl.cards.AbstractCard.RenderCustomDynamicVariable.myRenderDynamicVariable(this, tmp, $$);");
                }
            }
        });
        ...
        if (insertLine[0] > 0) {
            ctMethodToPatch.insertAt(insertLine[0]-1,
                    "if (tmp.length() != 4 && tmp.length() != 5) {" +
                            "start_x += basemod.patches.com.megacrit.cardcrawl.cards.AbstractCard.RenderCustomDynamicVariable.myRenderDynamicVariable(this, tmp, tmp.charAt(1), start_x, draw_y, i, font, sb, null);" +
                        "}");
        }
    }
